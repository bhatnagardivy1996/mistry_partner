...
class EServicesController extends GetxController {
  ...
  final providerId = Get.find<AuthService>().user.value.id.toString();
  EServicesController() {
  ...
  Future refreshEServices({bool showMessage}) async {
    ...
    await loadEServicesOfCategory(providerId: providerId, filter: selected.value);
    if (showMessage == true) {
  ...
  Future loadEServicesOfCategory({String providerId, CategoryFilter filter}) async {
    print("Prov:"+providerId.toString());
    try {
      ...
      switch (filter) {
        case CategoryFilter.ALL:
          _eServices = await _eProviderRepository.getEServices(eProviderId: providerId, page: this.page.value);
          break;
        case CategoryFilter.FEATURED:
          _eServices = await _eProviderRepository.getFeaturedEServices(eProviderId: providerId, page: this.page.value);
          break;
        case CategoryFilter.POPULAR:
          _eServices = await _eProviderRepository.getPopularEServices(eProviderId: providerId,page: this.page.value);
          break;
        case CategoryFilter.RATING:
          _eServices = await _eProviderRepository.getMostRatedEServices(eProviderId: providerId,page: this.page.value);
          break;
        case CategoryFilter.AVAILABILITY:
          _eServices = await _eProviderRepository.getAvailableEServices(eProviderId: providerId,page: this.page.value);
          break;
        default:
          _eServices = await _eProviderRepository.getEServices(eProviderId: providerId,page: this.page.value);
      }
